...
class Event(BaseAPI):
    def __init__(self):
        ...
        self.usgs_response = requests.get('https://earthquake.usgs.gov/fdsnws/event/1/query?format=geojson')
    ...
    def _extract_gdacs_data(self):
        if self.gdacs_response.status_code == 200:
            ...
            print("Failed to fetch GDACS data. Status code:", self.gdacs_response.status_code)
            return []
    ...
    def _extract_usgs_data(self):
        if self.usgs_response.status_code == 200:
            ...
            print("Failed to fetch USGS data. Status code:", self.usgs_response.status_code)
            return []
    ...
    def compare_and_store(self):
        ...
        for event in event_data:
            ...
            existing_event = self.collection.find_one({'source': event['source'], 
